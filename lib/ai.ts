...
export async function askAI(question: string, commodityId: string, commodityName: string): Promise<string> {
  try {
...
function generateMockResponse(question: string, commodityId: string, commodityName: string): string {
  ...
  else if (q.includes('grow') || q.includes('cultivat') || q.includes('farm') || q.includes('produc')) {
    if (commodityId === 'rice') {
  ...
  else if (q.includes('forecast') || q.includes('predict') || q.includes('future') || q.includes('trend')) {
    if (commodityId === 'rice') {
