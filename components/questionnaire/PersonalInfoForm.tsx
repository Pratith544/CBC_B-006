...
export default function PersonalInfoForm({
  ...
  const [experience, setExperience] = useState(
    formData.personalInfo?.experience || ""
  );
  const [location, setLocation] = useState(
    formData.personalInfo?.location || ""
  );
  const [landSize, setLandSize] = useState(
    formData.personalInfo?.landSize || ""
  );
  const [landUnit, setLandUnit] = useState(
    formData.personalInfo?.landUnit || "acres"
  );
  ...
  const validateForm = () => {
    ...
    else if (isNaN(Number(landSize)) || Number(landSize) <= 0)
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <select
              ...
              value={landUnit}
              onChange={(e) =>
                setLandUnit(e.target.value as "acres" | "hectares" | "bigha")
              }
              className="w-full px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-md focus:ring-2 focus:ring-green-500 focus:border-transparent dark:bg-gray-800 dark:text-white"
            >
