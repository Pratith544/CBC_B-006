...
export default function ResourceInfoForm({
  formData,
  updateFormData,
  onNext,
  onBack,
}: ResourceInfoFormProps) {
  const [hasEquipment, setHasEquipment] = useState(
    formData.resourceInfo?.hasEquipment || false
  );
  const [equipmentTypes, setEquipmentTypes] = useState(
    formData.resourceInfo?.equipmentTypes?.join(", ") || ""
  );
  const [laborAvailability, setLaborAvailability] = useState(
    formData.resourceInfo?.laborAvailability || "medium"
  );
  const [hasFertilizers, setHasFertilizers] = useState(
    formData.resourceInfo?.hasFertilizers || false
  );
  const [fertilizerTypes, setFertilizerTypes] = useState(
    formData.resourceInfo?.fertilizerTypes?.join(", ") || ""
  );
  const [marketAccess, setMarketAccess] = useState(
    formData.resourceInfo?.marketAccess || "good"
  );
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-6">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              setLaborAvailability(e.target.value as "high" | "low" | "medium")
            }
            className="w-full px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-md focus:ring-2 focus:ring-green-500 focus:border-transparent dark:bg-gray-800 dark:text-white"
          >
            <option value="high">High - Readily available when needed</option>
            <option value="medium">
              Medium - Available with some planning
            </option>
        ...
        <div className="bg-amber-50 dark:bg-amber-900/20 p-4 rounded-lg">
          ...
          <select
            ...
            onChange={(e) =>
              setMarketAccess(
                e.target.value as "excellent" | "good" | "limited" | "poor"
              )
            }
            className="w-full px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-md focus:ring-2 focus:ring-green-500 focus:border-transparent dark:bg-gray-800 dark:text-white"
          >
            <option value="excellent">
              Excellent - Direct access to multiple markets
            </option>
            <option value="good">
              Good - Reasonable access to local markets
            </option>
            <option value="limited">
              Limited - Some difficulty reaching markets
            </option>
            <option value="poor">
              Poor - Very difficult to access markets
            </option>
          </select>
        </div>
