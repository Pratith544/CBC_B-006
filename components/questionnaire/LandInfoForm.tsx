...
export default function LandInfoForm({
  formData,
  updateFormData,
  onNext,
  onBack,
  ...
  const [hasIrrigation, setHasIrrigation] = useState(
    formData.landInfo?.hasIrrigation || false
  );
  const [irrigationType, setIrrigationType] = useState(
    formData.landInfo?.irrigationType || ""
  );
  const [waterSource, setWaterSource] = useState(
    formData.landInfo?.waterSource || ""
  );
  const [waterAvailability, setWaterAvailability] = useState(
    formData.landInfo?.waterAvailability || "sufficient"
  );
  const [terrainType, setTerrainType] = useState(
    formData.landInfo?.terrainType || "flat"
  );
  const [previousCrops, setPreviousCrops] = useState(
    formData.landInfo?.previousCrops?.join(", ") || ""
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateFormData({
      landInfo: {
        ...
        waterAvailability: waterAvailability as
          | "abundant"
          | "sufficient"
          | "limited"
          | "scarce",
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-6">
        ...
        <div>
          ...
          <select
            ...
            className="w-full px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-md focus:ring-2 focus:ring-green-500 focus:border-transparent dark:bg-gray-800 dark:text-white"
          >
            <option value="abundant">
              Abundant - Available throughout the year
            </option>
            <option value="sufficient">
              Sufficient - Adequate for most crops
            </option>
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              setTerrainType(
                e.target.value as "flat" | "sloped" | "terraced" | "mixed"
              )
            }
            className="w-full px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-md focus:ring-2 focus:ring-green-500 focus:border-transparent dark:bg-gray-800 dark:text-white"
          >
